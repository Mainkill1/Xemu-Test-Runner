...
internal sealed partial class AgentJobStore
{
    ...
    public AgentTestSummary BakeTest(string testId, AgentBakeRequest request)
    {
        ...
        if (request.Description is { Length: > 240 }) throw new InvalidDataException("Test description must be at most 240 characters.");
        ...
        var definition = new AgentTestDefinition(testId, request.SourceJobId, request.Description ?? "", job,
            source.Request.Files.OrderBy(file => file.Path, StringComparer.Ordinal).ToArray(), builds);
        ...
        lock (_testLibraryGate)
        {
            ...
            if (File.Exists(path)) _ = ReadTest(testId, revision);
    ...
    public object DescribeTest(string id, string? revision, bool full)
    {
        ...
        var payload = EmulatorPayloadPath(baked.Definition.Job, baked.Definition.Files);
        return new
        {
            summary = TestSummary(baked), buildFiles = baked.Definition.BuildFiles,
            executable = baked.Definition.Job.Executable, emulator = payload,
            buildContractValid = baked.Definition.BuildFiles.Contains(payload, StringComparer.Ordinal),
            create = "/api/v1/jobs/from-test", definition = TestUrl(id, baked.Revision) + "?view=definition"
        };
    }
    ...
    public AgentOperation PrepareTest(AgentTestRunRequest request, CancellationToken lifetime)
    {
        ...
        if (payload != PortablePath(definition.Job.Executable) && replacements.Count > 0 && !replacements.Any(file => file?.Path == payload))
            throw BadRequest("emulator_payload_missing", "A launcher-bundle candidate replacement must include its native emulator.", "Supply all declared build slots, not just the wrapper.");
        ...
        PinEmulatorInput(job, files.Values.ToArray());
    ...
    {
        if (Directory.Exists(path) && (File.GetAttributes(path) & FileAttributes.ReparsePoint) != 0) throw new InvalidDataException("Linked test library directories are not supported.");
    }
    private static string RelativeInput(string package, string path) => System.IO.Path.GetRelativePath(package, JobDefinition.ResolveInsidePackage(package, path)).Replace('\\', '/');
    ...
    private static AgentTestSummary TestSummary(AgentBakedTest baked) => new(baked.Definition.Id, baked.Revision, baked.Definition.Description,
        baked.Definition.SourceJobId, baked.Definition.Job.Plan.Count, baked.Definition.Files.Count, baked.Definition.BuildFiles.Count, TestUrl(baked.Definition.Id, baked.Revision));
}
