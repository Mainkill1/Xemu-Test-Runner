...
public sealed partial class EmbeddedHttpServer
{
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                    ...
                    !HasVisiblePackage(_paths.Pending) && !HasVisiblePackage(_paths.Testing))
                {
                    ...
                    AgentJobs.AdvanceXisoCampaigns();
    ...
    private static bool HasVisiblePackage(string root) => Directory.Exists(root) && Directory.EnumerateDirectories(root).Any(path => !Path.GetFileName(path).StartsWith('.'));
    private async Task<bool?> TryRequestedTestRoutesAsync(Stream stream, HttpRequest request, CancellationToken ct)
    {
        ...
        { await WriteHtmlAsync(stream, TestConfigurationPage.Html, false, ct).ConfigureAwait(false); return false; }
        if (request.Path == "/api/v1/help" && GetQueryValue(request.Query, "topic") == "test-workflow" && request.Method == "GET")
        {
            ...
            }, cancellationToken: ct).ConfigureAwait(false); return false;
        }
        if (request.Path == "/api/v1/test-configs" && request.Method == "GET")
        {
            await WriteAgentJsonAsync(stream, AgentJobs.ListTests(ReadBoundedQuery(request.Query, "offset", 0, 0, 1000000), ReadBoundedQuery(request.Query, "limit", 25, 1, 100)), cancellationToken: ct).ConfigureAwait(false); return false;
        }
        ...
        if (request.Path.StartsWith(configs, StringComparison.Ordinal))
        {
            ...
            if (parts.Length == 1 && request.Method == "POST")
            {
                ...
                await WriteAgentJsonAsync(stream, AgentJobs.UploadConfig(Uri.UnescapeDataString(parts[0]), body), cancellationToken: ct).ConfigureAwait(false); return false;
            }
            if (parts.Length == 2 && request.Method == "GET")
            {
                if (!await EnsureOperationAllowedAsync(stream, "bulk_transfer", false, ct).ConfigureAwait(false)) return false;
                await WriteAgentJsonAsync(stream, AgentJobs.DescribeTest(Uri.UnescapeDataString(parts[0]), parts[1], true), cancellationToken: ct).ConfigureAwait(false); return false;
            }
        }
        ...
        if (request.Path == runs)
        {
            if (request.Method == "POST")
            {
                ...
                await WriteAgentJsonAsync(stream, AgentJobs.RequestedView(AgentJobs.CreateRequestedTest(body)), cancellationToken: ct).ConfigureAwait(false); return false;
            }
            if (request.Method == "GET")
            {
                await WriteAgentJsonAsync(stream, AgentJobs.ListRequestedTests(ReadBoundedQuery(request.Query, "offset", 0, 0, 1000000), ReadBoundedQuery(request.Query, "limit", 25, 1, 100)), cancellationToken: ct).ConfigureAwait(false); return false;
            }
        }
        if (request.Path.StartsWith(runs + "/", StringComparison.Ordinal))
        {
            ...
            if (parts.Length == 1 && request.Method is "GET" or "DELETE")
            {
                ...
                await WriteAgentJsonAsync(stream, AgentJobs.RequestedView(value), cancellationToken: ct).ConfigureAwait(false); return false;
            }
            if (parts.Length == 2 && parts[1] == "start" && request.Method == "POST")
            {
                ...
                await WriteAgentJsonAsync(stream, AgentJobs.RequestedView(AgentJobs.RequestTestStart(id)), 202, cancellationToken: ct).ConfigureAwait(false); return false;
            }
        }
        return null;
    }
