...
public sealed class GuestHddResultsDefinition : IJsonOnDeserialized
{
    ...
    public XisoExecution? Xiso { get; set; }
    void IJsonOnDeserialized.OnDeserialized() => Validate();
    public void Validate()
    {
        foreach (var path in new[] { Image, GuestPath, ExpectedResults })
            if (string.IsNullOrWhiteSpace(path) || path.Length > 512 || path.Contains('\\') || path.Contains(':') || path.Split('/').Any(part => part.Length == 0 || part is "." or ".." || part.Any(char.IsControl)))
                throw new InvalidDataException("Guest HDD paths must be non-empty relative forward-slash paths without traversal.");
        if (PartitionOffsetBytes < 0 || PartitionOffsetBytes % 512 != 0 || PartitionLengthBytes < 8192 || PartitionLengthBytes % 512 != 0 || PartitionOffsetBytes > (1L << 44) - PartitionLengthBytes)
            throw new InvalidDataException("Declare the actual FATX partition byte offset and length, aligned to 512 bytes.");
        if (MaximumResultBytes is < 1 or > 16 * 1024 * 1024) throw new InvalidDataException("MaximumResultBytes must be 1..16777216.");
        ...
        {
            Xiso.Validate();
            if (Xiso.Image != Image || Xiso.PartitionOffsetBytes != PartitionOffsetBytes || Xiso.PartitionLengthBytes != PartitionLengthBytes || GuestPath != "xemu_perf_tests/results.txt")
                throw new InvalidDataException("XISO extraction geometry/path must match its injected plan.");
        }
    }
}
