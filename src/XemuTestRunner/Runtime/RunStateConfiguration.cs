...
{
    public static async Task<(List<string> Arguments, string SemanticHash)> PrepareAsync(
        JobDefinition job, string executable, string workingDirectory, IReadOnlyList<string> arguments,
        string resultDirectory, string evidenceDirectory, RunStorageReport report, CancellationToken ct)
    {
        ...
        for (var i = 0; i < arguments.Count; i++)
        {
            ...
            if (item == "-config_path" || item.StartsWith("-config_path=", StringComparison.Ordinal))
            {
                ...
                if (item == "-config_path")
                {
                    if (++i == arguments.Count) throw new InvalidDataException("-config_path needs a filename.");
            ...
            {
                if (new[] { "-drive", "-hda", "-hdb", "-hdc", "-hdd", "-blockdev", "-readconfig" }
                    .Any(option => item == option || item.StartsWith(option + "=", StringComparison.Ordinal)))
        ...
        EnsureInside(package, source);
        RunStateInventory.NoLinks(source);
        var bytes = await ReadSmallAsync(source, ct).ConfigureAwait(false);
        report.ConfigSourcePath = source;
        report.ConfigSha256 = RunStateInventory.Hash(bytes);
        ...
        var performance = Table(model, "perf");
        performance["cache_shaders"] = policy.CacheShaders;
        // Hash the authored semantic values before replacing portable placeholders
        // with run-specific absolute paths. Equal procedures must remain equal.
        ...
        foreach (var name in new[] { "bootrom_path", "flashrom_path", "dvd_path", "hdd_path", "eeprom_path" })
        {
            ...
            if (!files.TryGetValue(name, out var raw) || raw is not string text || string.IsNullOrWhiteSpace(text))
            {
                if (mutable && policy.RequirePrivateGuestState)
                    throw new InvalidDataException("Managed private guest state requires explicit " + name + " and its materialized seed.");
            ...
            var path = Path.GetFullPath(text, workingDirectory);
            ...
            files[name] = path;
            report.StoragePaths[name] = path;
        }
        var screenshots = Path.Combine(resultDirectory, "screenshots");
        RunStateInventory.NoLinks(screenshots);
        Directory.CreateDirectory(screenshots);
        Table(model, "general")["screenshot_dir"] = screenshots;
        report.StoragePaths["screenshots"] = screenshots;
        ...
        report.EffectiveConfigPath = effectivePath;
        report.EffectiveConfigSha256 = RunStateInventory.Hash(effective);
        args.Add("-config_path"); args.Add(effectivePath);
        return (args, semanticHash);
    }
    ...
    {
        var path = Path.Combine(result, "runtime-state.json");
        ...
        var files = root.GetProperty("files").Deserialize<List<RuntimeFileMaterialization>>(ConfigLoader.JsonOptions)
            ?? throw new InvalidDataException("Missing runtime files.");
    ...
    internal static void AddUncontrolled(RunStorageReport report, string issue)
    {
        if (!report.Uncontrolled.Contains(issue)) report.Uncontrolled.Add(issue);
    }
}
