...
Cache state is an input. Declare RuntimeState.Isolation for managed benchmarks, pin shader-cache behavior, use private HDD/EEPROM seeds, and keep cold/seeded/inherited results separate. `runner_tests.py state RUN_ID` gives actual captured paths, before/after hashes/counts and unresolved controls without downloading the cache. Cold application cache does not mean cold driver/OS caches. AllowUncontrolledDriverCache is explicit acceptance of an unresolved factor, not verification. Do not silently enable it to obtain a green comparison. Historical results without state evidence are legacy, not certified cold. See docs/RUN-STATE.md; do not delete global caches or launch unrequested warm-up tests.
...
Large Xbox HDDs belong in the runner disk catalog, not duplicated inside every saved test. Use `runner_tests.py disk-list`, `disk-upload`, or `disk-import`; saved tests pin the asset ID and full SHA-256 through `RuntimeState.DiskAssets`. The default runner-managed disk retention is `deleteAfterEvidence`: after confirmed target exit, guest/crash/state evidence is finalized before the private runtime HDD is deleted. Do not retain boot-only XISO HDDs or copy them over SSH for diagnosis. Snapshot carriers are currently copied byte-for-byte per active run and loaded through the existing SnapshotName/`-loadvm` path; do not extract a QCOW2 internal snapshot and assume that preserves xemu's full VM save state. See docs/DISK-ASSETS.md.
...
Known remaining workflow gaps are recorded in docs/AGENT-WORKAROUND-AUDIT.md. Cache snapshot promotion, complete effective-default introspection and native driver-cache verification are also not implemented. Report missing capabilities rather than silently replacing them with queue-directory or shell manipulation.
