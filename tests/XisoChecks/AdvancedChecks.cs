...
internal static class AdvancedChecks
{
    public static async Task Run(Func<string, Func<Task>, Task> check,
        Func<AgentFixture, Task<JsonElement>> setup, Func<byte[]> seed)
    {
        await check("self-contained QCOW2 plan injection preserves valid cluster mappings", async () =>
        {
            ...
            var bytes = Qcow(seed());
            var original = DigestBytes(bytes);
        ...
        await check("shared QCOW2 slot refuses mutation and leaves its catalog seed intact", async () =>
        {
            ...
            var original = DigestBytes(bytes);
            SetDisk(host, definition, "shared-seed", bytes);
            var refused = false;
        ...
        await check("guest receipt and exact selected IDs gate subset results", async () =>
        {
            ...
            var plan = definition.Xiso!;
            var image = Path.Combine(runtime!.Directory, "hdd.qcow2");
            ...
            PutFile(bytes, 12352, "results.txt", 20, raw);
            PutFile(bytes, 12416, "resolved-plan-result.json", 21, receipt);
        ...
        await check("campaign continues after an archived crash without replaying it", async () =>
        {
            ...
            await Queued(host, "xc-continue-001");
            host.State.SetPhase("fixture");
            var source = Path.Combine(host.Paths.Pending, "agent-xc-continue-001");
            var target = Path.Combine(host.Paths.Tested, "agent-xc-continue-001");
            ...
            host.State.SetPhase("idle");
            await Queued(host, "xc-continue-002");
        ...
        await check("authorized campaign resumes an interrupted child creation handoff", async () =>
        {
            ...
            var plan = await host.Json("/api/v1/xiso-campaigns/handoff?view=plan");
            var chunk = plan.GetProperty("chunks")[0];
            ...
            host.State.SetPhase("idle");
            await Queued(host, "xc-handoff-001");
        });
    }
    ...
    private static byte[] Qcow(byte[] raw)
    {
        ...
        for (var i = 0; i < count; i++) U64(4 * cluster + i * 8, (1UL << 63) | (ulong)((i + 5) * cluster));
