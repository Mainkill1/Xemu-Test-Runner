...
internal static class SharedInputChecks
{
    ...
    private static async Task<(JobDefinition Job, string Executable, string Result, string Shared)> Setup(AgentFixture host, string bytes = "boot-rom")
    {
        ...
        using (var response = await host.Client.PutAsync("/api/v1/disk-assets/shared-boot/content", body))
        ...
        {
            Id = "shared-run", Executable = "xemu", RuntimeState = new { Enabled = true, Isolation = Policy(sha) }
        }));
        ...
        await File.WriteAllTextAsync(Path.Combine(runtime, "hdd.qcow2"), "disk");
        await File.WriteAllTextAsync(Path.Combine(runtime, "eeprom.bin"), "eeprom");
        AtomicJson.Write(Path.Combine(result, "runtime-state.json"), new
        {
            Directory = runtime, Files = new[]
            {
